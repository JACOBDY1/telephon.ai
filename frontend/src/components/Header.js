  ...
  return (
    <div className={`${darkMode ? 'bg-gray-900' : 'bg-white'} border-b border-gray-200 dark:border-gray-700 px-4 py-3 lg:px-6 lg:py-4`}>
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="hidden md:flex flex-1 max-w-md" dir="rtl">
          <button
            onClick={() => setShowGlobalSearch(true)}
            className="relative w-full"
          >
            <Search className="absolute right-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400" />
            <div className="w-full pr-10 pl-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800 text-gray-500 dark:text-gray-400 text-sm text-right hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
              {t?.search || 'חיפוש...'}
              <span className="float-left text-xs">⌘K</span>
            </div>
          </button>
        </div>
